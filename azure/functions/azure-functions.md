...
Les Azure Functions sont les éléments d'une ressource [[azure-function-app|Azure Function App]], affichés dans la section **Functions/Functions** du portail Azure, après avoir sélectionné la ressource Function App correspondante.
...
- Il est également possible d'**ajouter une fonction** Azure à la [[azure-function-app|Azure Function App]] **via Visual Studio**. Pour ce faire, il suffit de créer une nouvelle classe au projet, de type **"Azure function"**.
- Comme pour une Azure Web App, il est possible d'ajouter des **items de configuration** à une Azure Function (section "**Settings/Configuration**"), y compris des **connection strings**, dans l'onglet correspondant.
	Ces éléments de configuration seront accessibles dans le code de l'Azure Function **en tant que variables d'environnement**. Par exemple :
	```csharp
