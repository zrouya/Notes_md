...
Le Dockerfile est le **patron de construction** du conteneur, il est constitué de la liste des commandes, ou **stanza**, à exécuter pour lancer le conteneur. Il constitue le squelette de l'[[images-docker|image Docker]].
...
Chaque ligne (commande) du fichier docker donne lieu à un [[layers-images-docker|layer de l'image]], généré lors de la [[build-des-images-docker|construction de l'image Docker]]. Il est donc important, pour bénéficier au maximum du système de cache des layers, de **bien ordonner les stanza** dans le Dockerfile : les commandes **les moins susceptibles de changer en premier**, celles **les plus susceptibles de changer en dernier**.
